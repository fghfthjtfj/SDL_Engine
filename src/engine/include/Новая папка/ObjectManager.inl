...
namespace std {
    ...
    struct hash<std::tuple<T...>> {
        size_t operator()(const std::tuple<T...>& tpl) const noexcept {
            size_t h = 0;
            std::apply([&h](auto&&... args) {
                (..., (h ^= std::hash<std::decay_t<decltype(args)>>{}(args)+0x9e3779b9 + (h << 6) + (h >> 2)));
                }, tpl);
            return h;
        }
    };
...
Entity ObjectManager::create(const std::string& scene_name, Components&&... comps) {
...
template<typename T>
foreach_arg_t<T> ObjectManager::Get(SceneData* scene, Entity e)
{
