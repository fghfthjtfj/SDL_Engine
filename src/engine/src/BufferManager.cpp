...
BufferManager::BufferManager(SDL_GPUDevice* device) : ResourceManager(device) {
	...
	CreateBufferData(DEFAULT_POSITION_INDEX_BUFFER, BASE_TB_SIZE / 10/ 16, SDL_GPU_BUFFERUSAGE_GRAPHICS_STORAGE_READ, BufferDataType::Static);
    ...
    CreateBufferData(DEFAULT_INDIRECT_BUFFER, sizeof(SDL_GPUIndexedIndirectDrawCommand) * 10, SDL_GPU_BUFFERUSAGE_INDIRECT, BufferDataType::Static);
    CreateBufferData(DEFAULT_ENTITY_TO_BATCH_BUFFER, BASE_TB_SIZE / 10 / 16, SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_READ, BufferDataType::Static);
    CreateBufferData(DEFAULT_BOUND_SPHERE_BUFFER, BASE_TB_SIZE / 10/ 4, SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_READ, BufferDataType::Static);
    CreateBufferData(DEFAULT_COUNT_BUFFER, sizeof(SDL_GPUIndexedIndirectDrawCommand) * 1 * 6, SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_READ |SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_WRITE, BufferDataType::Dynamic);
    CreateBufferData(DEFAULT_OUT_TRANSFORM_BUFFER, sizeof(glm::mat4) * 10, SDL_GPU_BUFFERUSAGE_GRAPHICS_STORAGE_READ | SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_WRITE, BufferDataType::Static);
}
