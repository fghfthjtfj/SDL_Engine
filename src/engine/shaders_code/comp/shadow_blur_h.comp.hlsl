...
void main(uint3 tid : SV_DispatchThreadID)
{
    ...
    u_input.GetDimensions(0, w, h, elements, levels);
    ...
    float2 result = u_input.SampleLevel(u_sampler,
                       float3(uv, layerIndex), 0) * weights[0];

    [unroll]
    for (int i = 1; i < 5; ++i) {
        float2 off = float2(texel * i, 0);
        result += u_input.SampleLevel(u_sampler, float3(uv + off, layerIndex), 0) * weights[i];
        result += u_input.SampleLevel(u_sampler, float3(uv - off, layerIndex), 0) * weights[i];
    }
