...
float4 main(PSInput input, bool isFrontFace : SV_IsFrontFace) : SV_Target0
{
    ...
    float3 lightSum = float3(0.0, 0.0, 0.0);   // только прямой свет, без ambient
    ...
    for (uint i = 0; i < lightCount; ++i)
    {
        ...
        lightSum += LightBlock[i].color_power.rgb * intensity;
    }

    // Освещение не опускается ниже ambient: ambient — это пол, а не добавка сверху
